enyo.kind({
	...
	create: function() {
		...
		if(this.data != null){
			this.$.list.setCount(data.estudiantes.length);
			this.setupItem();
        }
	},
	setupItem: function(inSender, inEvent) {
			var i = inEvent.index;
			if (!this.names[i]) {
				this.names[i] = this.pasarJS(this.data.estudiantes[i].tag);
				this.numbers[i] = this.pasarJS(this.data.estudiantes[i].descripcion);
			}
	...
	listTap: function(inSender,inEvent){
		console.log("ubicacion: "+this.data.estudiantes[inEvent.index].ubicacion);
		console.log("presionado "+this.names[inEvent.index]);
		this.bubble("onItemTapEdificio",this.data.estudiantes[inEvent.index]);
		inEvent.preventDefault();
	},
	refreshList: function() {
			this.$.list.setCount(this.data.estudiantes.length);
			this.$.list.reset();
			this.$.list.refresh();
