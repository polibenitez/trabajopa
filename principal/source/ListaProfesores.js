enyo.kind({
	...
	setupItem: function(inSender, inEvent) {
			var i = inEvent.index;
			if (!this.names[i]) {
				this.names[i] = this.pasarJS("Ed_"+this.data.profesores[i].despacho);
				this.numbers[i] = this.pasarJS(this.data.profesores[i].nombre +" "+ this.data.profesores[i].apellido1);
			}
